...
impl DolrTreasury {
    async fn get_or_init(&mut self, storage: &Storage) -> &mut DolrTreasuryInner {
        if self.0.is_some() {
            return self.0.as_mut().unwrap();
        }

        let dolr_treasury_limit = storage.get("dolr-treasury-limit").await.unwrap_or_default();
        self.0 = Some(dolr_treasury_limit);
        self.0.as_mut().unwrap()
    }

    async fn treasury(&mut self, storage: &mut Storage) -> Result<&mut DolrTreasuryInner> {
        ...
        if treasury.last_reset_epoch >= Date::now().as_millis() + (24 * 3600 * 1000) {
            *treasury = DolrTreasuryInner::default();
            storage.put("dolr-treasury-limit", treasury.clone()).await?;
        };

        Ok(treasury)
    }

    pub async fn try_consume(&mut self, storage: &mut Storage, amount: Nat) -> Result<()> {
    ...
    pub async fn rollback(&mut self, storage: &mut Storage, amount: Nat) -> Result<()> {
        let treasury = self.treasury(storage).await?;
        treasury.amount =
    ...
    pub async fn amount(&mut self, storage: &mut Storage) -> Result<Nat> {
